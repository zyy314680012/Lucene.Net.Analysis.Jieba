        ...
        internal ISet<string> GetEdits1(string word)
        {
            ...
            if (word.Length > 1)
            {
                ...
                for (int i = 1; node.IsNotNull() && node.Children.IsNotEmpty() && i < word.Length; i++)
                {
                    ...
                    node = node.Children.GetDictionaryValueOrDefault(word[i]);
                }
            }
            ...
            if (word.Length > 1)
            {
                ...
                var node = WordTrie.Root.Children.GetDictionaryValueOrDefault(word[0]);
                for (int i = 0; node.IsNotNull() && node.Children.IsNotEmpty() && i < word.Length; i++)
                {
                    ...
                    if (i < word.Length - 1)
                    {
                        node = node.Children.GetDictionaryValueOrDefault(word[i + 1]);
                    }
                }
            }
