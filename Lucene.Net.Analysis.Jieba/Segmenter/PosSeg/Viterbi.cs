...
namespace Lucene.Net.Analysis.Jieba.Segmenter.PosSeg;
...
public class Viterbi
{
    ...
    {
        var startJson = FileExtension.ReadAllLine(ConfigManager.PosProbStartFile);

        _startProbs = JsonSerializer.Deserialize<IDictionary<string, double>>(startJson);

        var transJson = FileExtension.ReadAllLine(ConfigManager.PosProbTransFile);
        _transProbs = JsonSerializer.Deserialize<IDictionary<string, IDictionary<string, double>>>(transJson);

        var emitJson = FileExtension.ReadAllLine(ConfigManager.PosProbEmitFile);
        _emitProbs = JsonSerializer.Deserialize<IDictionary<string, IDictionary<char, double>>>(emitJson);

        var tabJson = FileExtension.ReadAllLine(ConfigManager.CharStateTabFile);
